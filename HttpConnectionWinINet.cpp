...
// ハンドルをクリア
void
HttpConnectionWinINet::closeHandle()
{
	LOCK;
 ...
 */
bool
HttpConnectionWinINet::open(const TCHAR *method,
					 const TCHAR *url,
					 const TCHAR *_user,
					 const TCHAR *_passwd) {
	LOCK;
 ...
 */
int
HttpConnectionWinINet::request(RequestCallback requestCallback, RetryCallback retryCallback, void *context)
{
	LOCK;
 ...
 */
void
HttpConnectionWinINet::queryInfo()
{
	LOCK;
 ...
 */
int
HttpConnectionWinINet::response(ResponseCallback callback, void *context)
{
	LOCK;
